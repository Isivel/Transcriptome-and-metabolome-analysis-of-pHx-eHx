...
#[1] "Intercept"           "condition_eHx_vs_pHx" "time_18_vs_0"        "time_36_vs_0"        "time_72_vs_0"       
#[6] "conditioneHx.time18" "conditioneHx.time36" "conditioneHx.time72"
...
contrasts$name <- c('18-0:pHx', '36-0:pHx', '72-0:pHx', '18-0:eHx', '36-0:eHx', '72-0:eHx',
                    '36-18:pHx', '72-18:pHx', '36-18:eHx', '72-18:eHx', '72-36:pHx', '72-36:eHx')

res_DEGs.time$contrast_alias <- contrasts$name[match(res_DEGs.time$contrast, contrasts$contrast)]

up_tap <- as.data.frame(table(res_DEGs.time$contrast_alias[res_DEGs.time$padj < 0.001 & res_DEGs.time$log2FoldChange > 1]))
contrasts$up <- up_tap$Freq[match(contrasts$name, up_tap$Var1)]

down_tap <- as.data.frame(table(res_DEGs.time$contrast_alias[res_DEGs.time$padj < 0.001 & res_DEGs.time$log2FoldChange < -1]))
contrasts$down <- down_tap$Freq[match(contrasts$name, down_tap$Var1)]

contrasts$sum <- contrasts$up + contrasts$down
write.table(contrasts, col.names = T, row.names = F, quote = F, sep = "\t", file = 'runtime/2.DEG/DEG_stat.time.tsv')

res_DEGs.time$log2FoldChange <- signif(res_DEGs.time$log2FoldChange, digits = 3)
res_DEGs.time$stat <- signif(res_DEGs.time$stat, digits = 3)
res_DEGs.time$lfcSE <- signif(res_DEGs.time$lfcSE, digits = 3)
res_DEGs.time$pvalue <- signif(res_DEGs.time$pvalue, digits = 3)
res_DEGs.time$padj <- signif(res_DEGs.time$padj, digits = 3)
write.table(res_DEGs.time, col.names = T, row.names = F, quote = F, sep = "\t", file = 'runtime/2.DEG/DEG.time.tsv')


## number of DEGs is 3729
load('runtime/2.DEG/res_DEGs.time.rda')
gene.DE <- unique(res_DEGs.time$geneID[which(res_DEGs.time$padj < 0.001 &
                                               abs(res_DEGs.time$log2FoldChange) > 1)])
...
pdf("runtime/2.DEG/heatmap.time.pdf", width = 7.5, height = 7)
out_time <- pheatmap::pheatmap(fpkm_tab[gene.DE,],
...
save(out_time, file = "runtime/2.DEG/pheatmap_Time.Rdata")

cluster_tab.time <- sort(cutree(out_time$tree_row, k = 8))
...
tmp_tab$geneSymbol <- res_DEGs.time$geneSymbol[match(tmp_tab$ID, rownames(res_DEGs.time))]
write.table(tmp_tab, col.names = T, row.names = F, sep = "\t", quote = F,
            file = "runtime/2.DEG/heatmap.time.cluster.txt")
...
ann_colors = list(
  ...
  Cluster = c('Cluster1' = palette[1], 'Cluster2' = palette[2], 'Cluster3' = palette[3], 'Cluster4' = palette[4],
              'Cluster5' = palette[5], 'Cluster6' = palette[6], 'Cluster7' = palette[7], 'Cluster8' = palette[8])
)

pdf("runtime/2.DEG/heatmap.time.withRowAnno.pdf", width = 7.5, height = 7)
out_time.new <- pheatmap::pheatmap(fpkm_tab[gene.DE,],
                                   clustering_method = "ward.D2",
                                   clustering_distance_rows = 'correlation',
                                   show_rownames =F,show_colnames =F,fontsize_row=7,
...
## trend graph
dat_expr <- fpkm_tab[names(cluster_tab.time),]
...
df_plot$cluster <- cluster_tab.time[match(df_plot$variable, names(cluster_tab.time))]
...
require(ggpubr)
pdf("runtime/2.DEG/DEG_expr.line.pdf",width = 17,height = 13, onefile = T)
ggarrange(plotlist = plot_list,ncol = 3, nrow = 3)
dev.off()

save(df_plot, file = 'runtime/2.DEG/data_plot_line.time.Rdata')

## save data frame with cluster information
res_cluster <- data.frame(geneID = rownames(fpkm_tab), stringsAsFactors = F)
res_cluster$Cluster <- paste0('Cluster', cluster_tab.time[match(res_cluster$geneID, names(cluster_tab.time))])
...
write.table(res_cluster, col.names = T, row.names = F, sep = "\t", quote = F, 
            file = "runtime/2.DEG/GEgene_cluster.time.tsv")
