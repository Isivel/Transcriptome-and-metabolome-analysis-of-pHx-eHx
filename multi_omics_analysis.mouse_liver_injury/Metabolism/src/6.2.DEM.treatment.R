...
outDir <- './runtime/7.DEM/Treatment'
...
compounds <- intersect(unique(res_DEM$Index[which(grepl('eHx-pHx', res_DEM$contrast) & res_DEM$q < 0.05)]),
                       c(rownames(vip1)[which(rowMeans(vip1[, 1:2]) > 1.5)], 
                         rownames(vip2)[which(rowMeans(vip2[, 1:2]) > 1.5)],
                         rownames(vip3)[which(rowMeans(vip3[, 1:2]) > 1.5)]))


plotDir <- 'runtime/7.DEM/Treatment'
## plot heatmap
...
pdf("runtime/7.DEM/Treatment/heatmap.treatment.pdf", width = 7.5, height = 7.5)
...
save(out_treatment, file = "runtime/7.DEM/Treatment/pheatmap_Treatment.Rdata")
...
cluster_tab.treatment <- sort(cutree(out_treatment$tree_row, k = 3))
...
write.table(tmp_tab, col.names = T, row.names = F, sep = "\t", quote = F,
            file = "runtime/7.DEM/Treatment/heatmap.treatment.cluster.txt")
...
ann_colors = list(
  Group = c("sham"="steelblue","eHx"="red", "pHx"="#FF7F00"),
  Time = c('0' = 'navy', '18' = '#ffcccb', '36' = 'salmon', '72' = 'darkred'),
  Cluster = c('Cluster1' = palette[1], 'Cluster2' = palette[2], 'Cluster3' = palette[3])
)
pdf("runtime/7.DEM/Treatment/heatmap.treatment.withCluster.pdf", width = 7.5, height = 7.5)
out_treatment <- pheatmap::pheatmap(dat.met.norm[compounds,],
                                    clustering_method = "ward.D2",
                                    clustering_distance_rows = 'correlation',
                                    show_rownames =F,show_colnames =F,fontsize_row=7,
...
meta_tab.treatment <- sort(cutree(out_treatment$tree_row, k = 3))

line_plot.list <- NULL
pie_plot.list <- NULL
for(clu in 1:3){
  compound.list <- all_data$Compounds[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == clu])]
  write.table(compound.list, row.names = F, col.names = F, quote = F, file = paste0('runtime/7.DEM/Treatment/compound_Treatment_cluster', clu,'.tsv'))
  
  compound_cluster <- all_data[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == clu]),]
  write.table(compound_cluster, row.names = T, col.names = T, quote = F, sep = "\t",
              file = paste0('runtime/7.DEM/Treatment/Treatment_cluster', clu,'.tsv'))
  ...
  ggsave(plot =  pie_plot.list[[paste0('Cluster', clu)]], width = 9, height = 6, filename = paste0('runtime/7.DEM/Treatment/compound_cluster', clu,'.pie.pdf'))
  ## trend line
  df_tmp <- as.data.frame(t(dat.met.norm.twA[names(meta_tab.treatment)[which(meta_tab.treatment == clu)],]))
  ...
  ggsave(plot = line_plot.list[[paste0('Cluster', clu)]], width = 6.5, height = 6, filename = paste0('runtime/7.DEM/Treatment/compound_cluster', clu,'_linePlot.pdf'))
  
}

require(ggpubr)
pdf("runtime/7.DEM/Treatment/DEM.line.pdf",width = 15,height = 4.5, onefile = T)
ggarrange(plotlist = line_plot.list,ncol = 3, nrow = 1)
dev.off()

pdf("runtime/7.DEM/Treatment/DEM.pie.pdf",width = 15,height = 4.5, onefile = T)
ggarrange(plotlist = pie_plot.list,ncol = 3, nrow = 1)
dev.off()
...
write.table(compound.list, row.names = F, col.names = F, quote = F, file = 'runtime/7.DEM/Treatment/compound_treatment_cluster1.tsv')

compound_cluster <- all_data[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == 1]),]
write.table(compound_cluster, row.names = T, col.names = T, quote = F, sep = "\t",
            file = 'runtime/7.DEM/Treatment/treatment_Cluster1.tsv')
df_plot.pie <- as.data.frame(table(compound_cluster$Class.I), stringsAsFactors = F)
df_plot.pie$lab <- paste0(df_plot.pie$Var1, ' (', df_plot.pie$Freq, ')')
df_plot.pie$lab <- factor(df_plot.pie$lab, levels = df_plot.pie$lab[order(df_plot.pie$Freq, decreasing = T)])
## pie
...
ggsave(plot = pie1, width = 9, height = 6, filename = 'runtime/7.DEM/Treatment/compound_Cluster1.pie.pdf')
## trend line
...
ggsave(plot = line_chart, width = 6.5, height = 6, filename = 'runtime/7.DEM/Treatment/Cluster1_linePlot.pdf')


###### cluster2
compound.list <- all_data$Compounds[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == 2])]
write.table(compound.list, row.names = F, col.names = F, quote = F, file = 'runtime/7.DEM/Treatment/compound_treatment_cluster2.tsv')

compound_cluster <- all_data[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == 2]),]
write.table(compound_cluster, row.names = T, col.names = T, quote = F, sep = "\t",
            file = 'runtime/7.DEM/Treatment/treatment_Cluster2.tsv')
df_plot.pie <- as.data.frame(table(compound_cluster$Class.I), stringsAsFactors = F)
df_plot.pie$lab <- paste0(df_plot.pie$Var1, ' (', df_plot.pie$Freq, ')')
df_plot.pie$lab <- factor(df_plot.pie$lab, levels = df_plot.pie$lab[order(df_plot.pie$Freq, decreasing = T)])
## pie
...
ggsave(plot = pie1, width = 9, height = 6, filename = 'runtime/7.DEM/Treatment/compound_Cluster2.pie.pdf')
## trend line
...
ggsave(plot = line_chart, width = 6.5, height = 6, filename = 'runtime/7.DEM/Treatment/Cluster2_linePlot.pdf')



###### cluster3
compound.list <- all_data$Compounds[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == 3])]
write.table(compound.list, row.names = F, col.names = F, quote = F, file = 'runtime/7.DEM/Treatment/compound_treatment_cluster3.tsv')

compound_cluster <- all_data[which(all_data$Index %in% names(meta_tab.treatment)[meta_tab.treatment == 3]),]
write.table(compound_cluster, row.names = T, col.names = T, quote = F, sep = "\t",
            file = 'runtime/7.DEM/Treatment/treatment_Cluster3.tsv')
df_plot.pie <- as.data.frame(table(compound_cluster$Class.I), stringsAsFactors = F)
df_plot.pie$lab <- paste0(df_plot.pie$Var1, ' (', df_plot.pie$Freq, ')')
df_plot.pie$lab <- factor(df_plot.pie$lab, levels = df_plot.pie$lab[order(df_plot.pie$Freq, decreasing = T)])
## pie
...
ggsave(plot = pie1, width = 9, height = 6, filename = 'runtime/7.DEM/Treatment/compound_Cluster3.pie.pdf')
## trend line
df_tmp <- as.data.frame(t(dat.met.norm.twA[names(meta_tab.treatment)[which(meta_tab.treatment == 3)],]))
df_tmp$sample <- rownames(df_tmp)
df_plot.tmp <- melt(df_tmp)
df_plot.tmp$Time <- df_calDiff$Time[match(df_plot.tmp$sample, df_calDiff$sample)]
df_plot.tmp$Group <- df_calDiff$Group[match(df_plot.tmp$sample, df_calDiff$sample)]
...
ggsave(plot = line_chart, width = 6.5, height = 6, filename = 'runtime/7.DEM/Treatment/Cluster3_linePlot.pdf')
...
for(clu in c('1', '2', '3')){
  ...
  res_DEM.sub <- res_DEM[res_DEM$Index %in% dat.compound_DE$Index & grepl('eHx-pHx', res_DEM$contrast), ]
  ...
  for(me in unique(res_DEM.sub$Index)[1:10]){
    ...
    ggsave(plot = boxplot, width = 4.5, height = 4, 
           filename = paste0('runtime/7.DEM/Treatment/Cluster',clu,'.', me, '_', num, '.boxplot.pdf'))
    num <- num + 1
