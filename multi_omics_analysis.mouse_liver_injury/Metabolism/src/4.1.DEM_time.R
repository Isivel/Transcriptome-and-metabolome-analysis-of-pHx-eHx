...
compounds <- unique(res_DEM$Index[which(grepl('^\\d', res_DEM$contrast, perl = T) & res_DEM$q < 0.05)])

plotDir <- 'runtime/4.DEM.new/Time'
## plot heatmap
...
pdf("runtime/4.DEM.new/Time/heatmap.Time.pdf", width = 7.5, height = 7.5)
...
save(out_Time, file = "runtime/4.DEM.new/Time/pheatmap_Time.Rdata")
...
meta_tab.Time <- sort(cutree(out_Time$tree_row, k = 5))


cluster_tab.time <- sort(cutree(out_Time$tree_row, k = 5))
...
sample_order<-order(annotation_col$Group, annotation_col$Time)
pdf("runtime/4.DEM.new/Time/heatmap.Time.withCluster.pdf", width = 7.5, height = 7.5)
out_Time <- pheatmap::pheatmap(dat.met.norm[compounds, ],
                               clustering_method = "ward.D2",
                               clustering_distance_rows = 'correlation',
                               show_rownames =F,show_colnames =F,fontsize_row=7,
...
for(clu in 1:5){
  ...
  write.table(compound.list, row.names = F, col.names = F, quote = F, file = paste0('runtime/4.DEM.new/Time/compound_Time_cluster', clu,'.tsv'))
  ...
  write.table(compound_cluster, row.names = T, col.names = T, quote = F, sep = "\t",
              file = paste0('runtime/4.DEM.new/Time/Time_cluster', clu,'.tsv'))
  ...
  ggsave(plot =  pie_plot.list[[paste0('Cluster', clu)]], width = 9, height = 6, filename = paste0('runtime/4.DEM.new/Time/compound_cluster', clu,'.pie.pdf'))
  ## trend line
  ...
  ggsave(plot = line_plot.list[[paste0('Cluster', clu)]], width = 6.5, height = 6, filename = paste0('runtime/4.DEM.new/Time/compound_cluster', clu,'_linePlot.pdf'))
  
}

require(ggpubr)
pdf("runtime/4.DEM.new/Time/DEM.line.pdf",width = 15,height = 9, onefile = T)
ggarrange(plotlist = line_plot.list,ncol = 3, nrow = 2)
dev.off()

pdf("runtime/4.DEM.new/Time/DEM.pie.pdf",width = 15,height = 9, onefile = T)
...
#### boxplot
dat.met.norm <- readRDS('runtime/1.data/all_sample.norm.RDS')
plotDir <- './runtime/4.DEM.new/Time/boxplot'
if(!dir.exists(plotDir)) dir.create(plotDir, recursive = TRUE)

for(clu in c('1', '2', '3', '4', '5')){
  ...
  for(me in unique(res_DEM.sub$Index)[1:10]){
    ...
    for(i in 1:nrow(add_loc)){
      ...
      if(grepl('0-18:eHx', add_loc$contrast[i])){
      ...
      }else if(grepl('0-18:pH', add_loc$contrast[i])){
        ...
        boxplot <- boxplot + annotate('text', x = 1 + (2.75 - 1)/2, 
          ...
          geom_segment(aes_string(x = 2.75, xend = 2.75, 
                           y = Y2, 
                           yend = Y1))
      }else if(grepl('0-36:pH', add_loc$contrast[i])){
        ...
        boxplot <- boxplot + annotate('text', x = 1 + (3.75 - 1)/2, 
          ...
          geom_segment(aes_string(x = 3.75, xend = 3.75, 
                           y = Y2, 
                           yend = Y1))
      }else if(grepl('0-72:pH', add_loc$contrast[i])){
        ...
        boxplot <- boxplot + annotate('text', x = 1.75 + (2.75 - 1.75)/2, 
          ...
          geom_segment(aes_string(x = 2.75, xend = 2.75, 
                           y = Y2, 
                           yend = Y1))
      }else if(grepl('18-36:pH', add_loc$contrast[i])){
        ...
        boxplot <- boxplot + annotate('text', x = 1.75 + (3.75 - 1.75)/2, 
          ...
          geom_segment(aes_string(x = 3.75, xend = 3.75, 
                           y = Y2, 
                           yend = Y1))
      }else if(grepl('18-72:pH', add_loc$contrast[i])){
        ...
        boxplot <- boxplot + annotate('text', x = 2.75 + (3.75 - 2.75)/2, 
          ...
          geom_segment(aes_string(x = 3.75, xend = 3.75, 
                           y = Y2, 
                           yend = Y1))
      }else if(grepl('36-72:pH', add_loc$contrast[i])){
    ...
    ggsave(plot = boxplot, width = 4.5, height = 4.5, 
           filename = paste0('runtime/4.DEM.new/Time/boxplot/Cluster',clu,'.', me, '_', num, '.boxplot.pdf'))
    num <- num + 1
